...
export async function AdminPage({
  ...
  const selectedWork =
    adminView === "edit" && path
      ? works.find((work) => work.path === path)
      : undefined;
    ...
    <section className="space-y-5">
      ...
      {databaseStatus.ok && (
        <div className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(32rem,1.35fr)]">
          <MusicListPanel
            query={query}
            selectedPath={selectedWork?.path}
            works={filteredWorks}
          />
    ...
    <Card>
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <a
            href="/admin?view=new"
            className="inline-flex h-9 items-center justify-center rounded-md border border-zinc-700 bg-zinc-100 px-3 text-sm font-medium text-zinc-950 shadow-none transition-colors hover:bg-white"
          >
        ...
        {works.length ? (
          <div className="divide-y divide-zinc-800 border border-zinc-800">
            {works.map((work) => (
              <a
                key={work.path}
                href={`/admin?view=edit&path=${encodeURIComponent(work.path)}`}
                className={`grid gap-2 px-4 py-3 transition hover:bg-zinc-900 ${
                  selectedPath === work.path ? "bg-zinc-900" : ""
                }`}
              >
...
function MusicWorkForm({ work }: { work?: AdminMusicWork }) {
  return (
    <form action={saveMusicWorkAction} className="grid gap-5">
      ...
      <Step
        description="Set the URL slug and the visible song title."
        step="1"
        title={ADMIN_COPY.form.identityStep}
      >
